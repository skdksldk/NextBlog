...
const SideNav = () => {
    ...
    // contexts
    let { userAuth: { access_token, new_notifications_available } } = useContext(UserContext);
            ...
            <section className="relative flex gap-10 py-0 m-0 max-md:flex-col">
                ...
                <div className="sticky top-[80px] z-30">
                    <div className="md:hidden bg-white py-1 border-b border-grey flex flex-nowrap overflow-x-auto">
                        <button ref={dashboardNavIcon} className="p-5 capitalize" onClick={changePageState}><i className="fi fi-br-bars-staggered pointer-events-none"></i></button>
                        <button ref={dashboardActiveTabBtn} className="p-5 capitalize" onClick={changePageState}>{ pageState }</button>
                        <hr ref={activeTabLine} className="absolute bottom-0 duration-500" />
                    </div>

                    <div className={"min-w-[200px] h-cover md:sticky top-24 overflow-y-auto p-6 md:pr-0 md:border-grey md:border-r absolute max-md:top-[64px] bg-white max-md:w-[calc(100%+80px)] max-md:px-16 max-md:-ml-7 duration-500 " + (!sideNav ? "max-md:opacity-0 max-md:pointer-events-none" : "opacity-100 pointer-events-auto")}>
